...
async function loadSickBay(){
 ...
 try{
  ...
  const profs=(ids.length?(await supabase.from('profiles').select('user_id,display_name,nickname').in('user_id',ids)).data||[]:[]).slice().sort((a,b)=>(a.user_id===session.user.id?-1:b.user_id===session.user.id?1:0))
